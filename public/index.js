...
const renderAlbums = (album) => {
 ...
 const addToFavorites = async (e,album) => {
  ...
  try{
    await axios.put(`https://copia-intro-vercel.app/user/${user._id}`,{favorites})
    const star = e.target
...
const getAlbums =  async () => {
  try{
  const response = await axios.get('https://copia-intro-vercel.app/band')
...
getAlbums()
  ...
  const deleteAlbum = async (album) => {
    try{
      await axios.delete(`https://copia-intro-vercel.app/band/${album._id}`)
      swal({
        title: 'Success!',
